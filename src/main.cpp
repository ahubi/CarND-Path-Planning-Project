...
void fill_waypoints(vector<double>& _x,
                    vector<double>& _y,
                    vector<double>& _s,
                    vector<double>& _dx,
                    vector<double>& _dy)
{
  ...
  if(!in_map_){
    cerr << "Failed to read map data from " << map_file_
        << " fatal error!" << endl;
    exit(-1);
  }
  string line;
  ...
  if (length && length > 2 && data[0] == '4' && data[1] == '2') {
    ...
    if (s != "") {
      ...
      if (event == "telemetry") {
        ...
        // Sensor Fusion Data, a list of all other cars on the same side of the
        // road.
        ...
        for (int i = 0; i < (50 - prev_path_size); i++) {
          ...
          double out_x =
              ref_x + (new_x * cos(ref_yaw)) - (new_y * sin(ref_yaw));
          double out_y =
              ref_y + (new_x * sin(ref_yaw)) + (new_y * cos(ref_yaw));
